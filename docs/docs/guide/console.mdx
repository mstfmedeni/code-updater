...
Opens a localhost server based on plugins configured in `code-updater.config.ts`.
...
You can configure additional console settings in the `code-updater.config.ts` file.
...
You can set the git URL in the `code-updater.config.ts` file.
...
export default defineConfig({
  console: {
    gitUrl: "<your-repo>", // e.g. https://github.com/mstfmedeni/code-updater // [!code hl]
  },
});
```
