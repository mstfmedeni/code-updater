...
## Checklist
1. Ensure `code-updater.config.ts` is properly configured.
2. Wrap your components using `CodeUpdater.wrap`.
...
<PackageManagerTabs command={
  {
    npm: "npx code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\"",
    pnpm: "pnpm code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\"",
    yarn: "yarn code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\"",
  }
} />
...
<PackageManagerTabs command={
  {
    npm: "npx code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\" -f",
    pnpm: "pnpm code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\" -f",
    yarn: "yarn code-updater deploy -p <\"ios\" | \"android\"> -t \"1.x.x\" -f",
  }
} />
