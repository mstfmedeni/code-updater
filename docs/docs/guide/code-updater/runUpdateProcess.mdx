...
The `CodeUpdater.runUpdateProcess` function allows you to manually check for and apply updates at any point in your application’s lifecycle. This flexibility is useful when updates need to align with specific user flows or app states, rather than being triggered automatically at the entry point.
...
Here's an example of how to use `CodeUpdater.runUpdateProcess` to check for updates and handle them appropriately:
...
`CodeUpdater.runUpdateProcess` returns an object with the following properties:
