...
`CodeUpdater.wrap` checks for updates at the entry point, and if there is a bundle to update, it downloads the bundle and applies the update strategy.

## Usage
Use the `source` option in `CodeUpdater.wrap` to specify the update server URL, then wrap the App entry point.
...
export default CodeUpdater.wrap({ // [!code hl:7]
...
export default CodeUpdater.wrap({ // [!code hl:22]
  source: "<your-update-server-url>",
...
export default CodeUpdater.wrap({
  ...
  onUpdateProcessCompleted: ({ isBundleUpdated }) => {
    // if isBundleUpdated is true, `CodeUpdater.reload()` or restart the app to apply the new bundle
