...
class CodeUpdater : ReactPackage {
    ...
    override fun createNativeModules(context: ReactApplicationContext): MutableList<NativeModule> =
        listOf(CodeUpdaterModule(context)).toMutableList()
    ...
    companion object {
        ...
        private fun setBundleURL(
            ...
            val sharedPreferences =
                context.getSharedPreferences("CodeUpdaterPrefs", Context.MODE_PRIVATE)
            with(sharedPreferences.edit()) {
                putString("CodeUpdaterBundleURL", bundleURL)
                apply()
            }
        ...
        private fun extractZipFileAtPath(
            ...
            try {
                ...
                Log.d("CodeUpdater", "Failed to unzip file: ${e.message}")
                false
            }
        ...
        fun getJSBundleFile(context: Context): String {
            val sharedPreferences =
                context.getSharedPreferences("CodeUpdaterPrefs", Context.MODE_PRIVATE)
            val urlString = sharedPreferences.getString("CodeUpdaterBundleURL", null)
        ...
        suspend fun updateBundle(
            ...
            Log.d("CodeUpdater", "updateBundle bundleId $bundleId zipUrl $zipUrl")
            ...
            if (finalBundleDir.exists()) {
                Log.d("CodeUpdater", "Bundle for bundleId $bundleId already exists. Using cached bundle.")
                ...
                withContext(Dispatchers.IO) {
                    ...
                    val conn =
                        try {
                            ...
                            Log.d("CodeUpdater", "Failed to open connection: ${e.message}")
                            tempDir.deleteRecursively()
                    ...
                    try {
                        ...
                        if (totalSize <= 0) {
                            Log.d("CodeUpdater", "Invalid content length: $totalSize")
                            tempDir.deleteRecursively()
                            return@withContext false
                        }
                        ...
                        Log.d("CodeUpdater", "Failed to download data from URL: $zipUrl, Error: ${e.message}")
                        tempDir.deleteRecursively()
                        return@withContext false
                    } finally {
                        conn.disconnect()
                    ...
                    if (!extractZipFileAtPath(tempZipFile.absolutePath, extractedDir.absolutePath)) {
                        Log.d("CodeUpdater", "Failed to extract zip file.")
                        tempDir.deleteRecursively()
                        return@withContext false
                    }
                    true
            ...
            if (indexFileExtracted == null) {
                Log.d("CodeUpdater", "index.android.bundle not found in extracted files.")
                tempDir.deleteRecursively()
                return false
            }
            ...
            if (finalIndexFile == null) {
                Log.d("CodeUpdater", "index.android.bundle not found in final directory.")
                tempDir.deleteRecursively()
                return false
            }
            ...
            Log.d("CodeUpdater", "Setting bundle URL: $bundlePath")
            ...
            Log.d("CodeUpdater", "Downloaded and extracted file successfully.")
            return true
        }
        ...
        private fun cleanupOldBundles(bundleStoreDir: File) {
            ...
            if (sortedBundles.size > 2) {
                sortedBundles.drop(2).forEach { oldBundle ->
                    Log.d("CodeUpdater", "Removing old bundle: ${oldBundle.name}")
