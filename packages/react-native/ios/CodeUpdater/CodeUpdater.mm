...
- (void)setBundleURL:(NSString *)localPath {
    ...
    [defaults setObject:localPath forKey:@"CodeUpdaterBundleURL"];
    [defaults synchronize];
}

+ (NSURL *)cachedURLFromBundle {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *savedURLString = [defaults objectForKey:@"CodeUpdaterBundleURL"];
...
- (void)updateBundle:(NSString *)bundleId zipUrl:(NSURL *)zipUrl completion:(void (^)(BOOL success))completion {
    ...
    __weak CodeUpdater *weakSelf = self;
...
RCT_EXPORT_METHOD(reload) {
    NSLog(@"CodeUpdater requested a reload");
    dispatch_async(dispatch_get_main_queue(), ^{
        [super.bridge setValue:[CodeUpdater bundleURL] forKey:@"bundleURL"];
        RCTTriggerReloadCommandListeners(@"CodeUpdater requested a reload");
    });
}
...
- (std::shared_ptr<facebook::react::TurboModule>)getTurboModule:(const facebook::react::ObjCTurboModule::InitParams &)params {
    return std::make_shared<facebook::react::NativeCodeUpdaterSpecJSI>(params);
}
#endif
