...
// Template file: code-updater.config.ts
...
const options = {
  ...
  credentials: {
    ...
    sessionToken: process.env.CODE_UPDATER_S3_SESSION_TOKEN!,
  },
};
...
export async function createOrSelectIamRole({
  ...
  const { SDK } = await import("@code-updater/aws/sdk");
  ...
  const roleName = "code-updater-edge-role";
...
export const deployLambdaEdge = async (
  ...
  const lambdaName = await p.text({
    ...
    defaultValue: "code-updater-edge",
    placeholder: "code-updater-edge",
  });
  if (p.isCancel(lambdaName)) process.exit(1);

  // 2. Create zip file
  const lambdaPath = require.resolve("@code-updater/aws/lambda");
  ...
  await p.tasks([
      ...
      task: async (message) => {
        try {
          ...
          const createResp = await lambdaClient.createFunction({
            ...
            Description: "Code Updater Lambda@Edge function",
            Publish: true,
          });
    ...
    },
    {
      ...
      task: async () => {
        ...
        // ex) "arn:aws:lambda:us-east-1:123456789012:function:code-updater-edge:2"
...
export const createCloudFrontDistribution = async (
  ...
  const distributionConfig: DistributionConfig = {
    CallerReference: dayjs().format(),
    Comment: "Code Updater CloudFront distribution",
    Enabled: true,
    Origins: {
      Quantity: 1,
      Items: [
...
export const initAwsS3LambdaEdge = async () => {
  ...
  if (bucketName === createKey) {
    const name = await p.text({
      ...
      defaultValue: "code-updater-storage",
      placeholder: "code-updater-storage",
    });
    if (p.isCancel(name)) {
      process.exit(1);
    }
  ...
  if (mode === "sso") {
    await fs.writeFile("code-updater.config.ts", CONFIG_TEMPLATE_WITH_SESSION);
  } else {
    await fs.writeFile("code-updater.config.ts", CONFIG_TEMPLATE);
  }
  ...
  await makeEnv({
    CODE_UPDATER_S3_BUCKET_NAME: bucketName,
    CODE_UPDATER_S3_REGION: region,
    CODE_UPDATER_S3_ACCESS_KEY_ID: {
      comment,
      value: credentials?.accessKeyId ?? "",
    },
    CODE_UPDATER_S3_SECRET_ACCESS_KEY: {
      comment,
      value: credentials?.secretAccessKey ?? "",
    },
    ...(mode === "sso" && {
      CODE_UPDATER_S3_SESSION_TOKEN: credentials.sessionToken,
    }),
  });
  p.log.success("Generated '.env' file with AWS settings.");
  p.log.success("Generated 'code-updater.config.ts' file with AWS settings.");
  ...
  p.log.message(
    `Next step: ${link("https://mstfmedeni.github.io/code-updater/guide/providers/3_aws-s3-lambda-edge.html#step-4-changeenv-file-optional")}`,
  );
  p.log.success("Done! 🎉");
};
