...
import { copyDirToTmp } from "@code-updater/plugin-core";
...
export const initSupabase = async () => {
  ...
  const { supabaseApi } = await import("@code-updater/supabase");
  ...
  const supabasePath = path.resolve(
    require.resolve("@code-updater/supabase"),
    "..",
    "..",
  );
  ...
  await makeEnv({
    CODE_UPDATER_SUPABASE_ANON_KEY: serviceRoleKey.api_key,
    CODE_UPDATER_SUPABASE_BUCKET_NAME: bucketId,
    CODE_UPDATER_SUPABASE_URL: `https://${project.id}.supabase.co`,
  });
  p.log.success("Generated '.env' file with Supabase settings.");
  p.log.success(
    "Generated 'code-updater.config.ts' file with Supabase settings.",
  );
  ...
  p.log.message(
    `Next step: ${link(
      "https://mstfmedeni.github.io/code-updater/guide/getting-started/quick-start-with-supabase.html#step-4-add-codeupdater-to-your-project",
    )}`,
  );
  p.log.success("Done! 🎉");
};
