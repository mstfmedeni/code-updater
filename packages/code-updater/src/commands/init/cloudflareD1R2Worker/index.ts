...
import { copyDirToTmp, getCwd } from "@code-updater/plugin-core";
...
const deployWorker = async (
  ...
  const cwd = getCwd();
  const workerPath = require.resolve("@code-updater/cloudflare/worker", {
    paths: [cwd],
  });
  ...
  try {
    const { createWrangler } = await import("@code-updater/cloudflare/utils");
    ...
    const workerName = await p.text({
      ...
      defaultValue: "code-updater",
      placeholder: "code-updater",
    });
...
export const initCloudflareD1R2Worker = async () => {
  ...
  if (!apiToken) {
    p.log.warn(
      "Skipping API Token. You can set it later in .env CODE_UPDATER_CLOUDFLARE_API_TOKEN file.",
    );
  }
  ...
  await makeEnv({
    CODE_UPDATER_CLOUDFLARE_API_TOKEN: apiToken,
    CODE_UPDATER_CLOUDFLARE_ACCOUNT_ID: accountId,
    CODE_UPDATER_CLOUDFLARE_R2_BUCKET_NAME: selectedBucketName,
    CODE_UPDATER_CLOUDFLARE_D1_DATABASE_ID: selectedD1DatabaseId,
  });
  p.log.success("Generated '.env' file with Cloudflare settings.");
  p.log.success(
    "Generated 'code-updater.config.ts' file with Cloudflare settings.",
  );
  ...
  p.log.message(
    `Next step: ${link(
      "https://mstfmedeni.github.io/code-updater/guide/providers/2_cloudflare.html#step-4-add-codeupdater-to-your-project",
    )}`,
  );
  p.log.success("Done! 🎉");
};
